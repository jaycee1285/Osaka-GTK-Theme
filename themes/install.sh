...
OPTIONS:
  ...
  --tweaks                Specify versions for tweaks
                          1. [solarized]  Solarized ColorSchemes version
	...
	--tweaks)
		...
		for variant in $@; do
			...
			solarized)
				solarized="true"
				ctype="-Solarized"
				echo -e "Solarized ColorScheme version! ..."
				shift
				;;
			black)
				blackness="true"
...
solarized_color() {
	sed -i "/\@import/s/color-palette-default/color-palette-solarized/" "${SRC_DIR}/sass/_tweaks-temp.scss"
	sed -i "/\$colorscheme:/s/default/solarized/" "${SRC_DIR}/sass/_tweaks-temp.scss"
}
